...
const UploadImageScreen = ({ navigation, route }: any) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        `https://api.mobiholder.tech/api/users/auth/register/individual`,
        {
          ...data,
          dateOfBirth:
            data.dateOfBirth === "Invalid date" ? "" : data.dateOfBirth,
          photo: imageUrl,
        }
      );
      console.log(data);
    ...
    <PageContainer>
      <SafeAreaView style={tw`flex-1`}>
        ...
        <View style={tw`my-6 mb-10 `}>
          <Header size={21}>Upload Profile Image to proceed</Header>
